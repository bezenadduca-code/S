...
local function stamStop()
    stam.on = false
    if stam.thread then task.cancel(stam.thread);
    stam.thread = nil end
end
...
secStamina:Toggle({ Title = "Infinite Stamina", Type = "Checkbox", Flag = "stamNoLoss", Default = stam.noLoss,
    Callback = function(on)
        stam.noLoss = on;
        stamApply()
        if on and not stam.on then stam.on = true;
        stamStart() end
    end
})
...
do
    ...
    function ChatLogger.createUI()
        ...
        pcall(function()
            ...
            corner.CornerRadius = UDim.new(0, 8); corner.Parent = chatWindow
            ...
            titleBar.BorderSizePixel = 0; titleBar.Parent = chatWindow
            local tc = Instance.new("UICorner");
            tc.CornerRadius = UDim.new(0, 8); tc.Parent = titleBar
            ...
            ic.CornerRadius = UDim.new(0, 6); ic.Parent = inputFrame
            ...
            ic2.CornerRadius = UDim.new(0, 4); ic2.Parent = chatInput
            ...
            sc.CornerRadius = UDim.new(0, 4); sc.Parent = sendBtn
            ...
            titleBar.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    dragging = true;
                    dragStart = input.Position; startPos = chatWindow.Position
    ...
    function ChatLogger.addMessage(sender, message, messageType)
        pcall(function()
            ...
            local prefix = ""
            if messageType == "system" then nameColor = COLORS.System;
            prefix = "[sys] "
            elseif messageType == "whisper" then nameColor = COLORS.Whisper;
            prefix = "[pm] "
            elseif messageType == "team" then nameColor = COLORS.Team;
            prefix = "[team] "
            ...
            line.BackgroundTransparency = 1; line.TextColor3 = textColor;
            line.TextSize = 12
    ...
    function ChatLogger.setup()
        pcall(function()
            ...
            if tcs and tcs.TextChannels then
                ...
                table.insert(chatConnections, tcs.TextChannels.ChildAdded:Connect(function(ch) task.wait(0.1);
                hookChannel(ch) end))
            end
    ...
    function ChatLogger.cleanup()
        ...
        chatWindow = nil; chatScrollFrame = nil;
        chatContainer = nil; chatInput = nil; msgOrder = 0
    end
end
...
local function hbGetRemote()
    ...
    if ok and re then _hbRemote = re;
    return re end
    return nil
end
...
local function speedStop()
    ...
    local m = speedModule();
    if m then m.SprintSpeed = 26; pcall(function() m.MaxSprintSpeed = 26 end) end
end
lp.CharacterAdded:Connect(function()
...
local function flowNeighbour(r1,c1,r2,c2)
    ...
    if r2==r1 and c2==c1+1 then return"right" end; return false
end
...
-- The module returns the u61 class table;
-- hook u61.new to intercept new puzzle instances
do
...
-- Existing standard SAKIWARE ESP configuration setup runs below
local esp = {
    killers = false,
    survivors = false,
    generators = false,
        ...
        if isChar then
            ...
            if hum then
                ...
                local c=hum.HealthChanged:Connect(function()
                    if lbl.Parent then lbl.Text=obj.Name.." ("..math.floor(hum.Health/hum.MaxHealth*100).."%)"; end
                end)
...
secESP:Toggle({ Title = "Killers", Type = "Checkbox", Flag = "espKillers", Default = esp.killers, Callback = function(on) esp.killers = on; espDoKillers(on) end })
secESP:Toggle({ Title = "Survivors", Type = "Checkbox", Flag = "espSurvivors", Default = esp.survivors, Callback = function(on) esp.survivors = on; espDoSurvivors(on) end })
secESP:Toggle({ Title = "Generators", Type = "Checkbox", Flag = "espGenerators", Default = esp.generators, Callback = function(on) esp.generators = on; espDoGenerators(on) end })
secESP:Toggle({ Title = "Items (Cola/Med)", Type = "Checkbox", Flag = "espItems", Default = esp.items, Callback = function(on) esp.items = on; espDoItems(on) end })
secESP:Toggle({ Title = "Buildings/Sentry", Type = "Checkbox", Flag = "espBuildings", Default = esp.buildings, Callback = function(on) esp.buildings = on; espDoBuildings(on) end })


------------------------------------------------------------------------
-- NEW ADDED SUB-SECTION: FORSAKEN DOCUMENT / RING ESP
------------------------------------------------------------------------
local secForsakenESP = tabVisual:Section({ Title = "Forsaken Special ESP", Opened = true })
...
local function makeESP(obj)
    if CurrentESP or not fskEnabled then return end
...
svc.WS.DescendantAdded:Connect(function(v)
    task.wait(0.1)
    if fskEnabled and not CurrentESP then
        scan(v)
    end
end)
------------------------------------------------------------------------
