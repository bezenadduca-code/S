...
------------------------------------------------------------------------
-- CONFIG SHARE (URL-based)
------------------------------------------------------------------------

-- Reusable: apply a flags table into win.Flags AND restart all features
local function applySyncedFlags(flags)
    -- write every key into win.Flags so WindUI picks them up
    ...
    print("[hutao-cfg] applySyncedFlags complete.")
end

local sec_cfg = tabInterface:Section({ Title = "Config Share", Opened = true })

-- EXPORT: encode flags → upload to pastee.dev → copy URL to clipboard
sec_cfg:Button({ Title = "📤 Export Config (Copy URL)", Callback = function()
    task.spawn(function()
        local ok, result = pcall(function()
            local encoded = svc.Http:JSONEncode(win.Flags or {})

            -- Upload to pastee.dev anonymously
            local body = svc.Http:JSONEncode({ description = "hutao-forsaken config", sections = {{ name = "config", contents = encoded }} })
            local resp = svc.Http:RequestAsync({
                Url     = "https://api.pastee.dev/v1/pastes",
                Method  = "POST",
            ...
            if not resp.Success then
                ...
                pcall(function() ui:Notify({ Title = "Upload failed", Content = "Raw JSON copied instead.", Duration = 4 }) end)
                return
            end

            local respData = svc.Http:JSONDecode(resp.Body)
            -- returns { id, link } — we store the paste id so we can fetch via GET later
            local pasteId = respData.id or ""
            local shareUrl = "https://api.pastee.dev/v1/pastes/" .. pasteId

            if setclipboard then setclipboard(shareUrl) end
            pcall(function() ui:Notify({ Title = "✅ Config URL Copied!", Content = shareUrl, Duration = 6 }) end)
            print("[hutao-cfg] Exported to:", shareUrl)
        end)
        if not ok then
            warn("[hutao-cfg] Export error:", tostring(result))
            pcall(function() ui:Notify({ Title = "Export Error", Content = tostring(result), Duration = 4 }) end)
        end
    end)
end })

-- IMPORT: paste URL → HttpGet raw JSON → decode → apply
local _importURL = ""
sec_cfg:Input({
    Title       = "Config URL",
    Placeholder = "Paste config URL here...",
    Flag        = "cfgImportURL",
    Callback    = function(val) _importURL = val end,
})

sec_cfg:Button({ Title = "⬇ Load Config from URL", Callback = function()
    task.spawn(function()
        if _importURL == "" then
            pcall(function() ui:Notify({ Title = "No URL", Content = "Paste a config URL first.", Duration = 3 }) end)
            return
        end

        local ok, result = pcall(function()
            -- fetch the paste JSON from pastee.dev API
            local raw = game:HttpGet(_importURL, true)
            if not raw or raw == "" then
                error("Empty response from URL")
            end

            -- response is { success, paste: { sections: [{ contents }] } }
            local parsed = svc.Http:JSONDecode(raw)
            local contents
            if parsed.paste and parsed.paste.sections and parsed.paste.sections[1] then
                -- standard GET /v1/pastes/<id> response
                contents = parsed.paste.sections[1].contents
            elseif type(parsed) == "table" and not parsed.paste then
                -- already a flat flags table (raw JSON fallback)
                contents = raw
            else
                error("Could not find config contents in paste response")
            end

            local flags = svc.Http:JSONDecode(contents)
            if type(flags) ~= "table" then
                error("Config contents are not a valid table")
            end

            -- restore everything exactly like the startup load does
            applySyncedFlags(flags)

            -- also persist it locally so it survives restarts
            pcall(function() sakiConfig:Save() end)

            pcall(function() ui:Notify({ Title = "✅ Config Loaded!", Content = "All settings restored from URL.", Duration = 4 }) end)
        end)

        if not ok then
            warn("[hutao-cfg] Import error:", tostring(result))
            pcall(function() ui:Notify({ Title = "Load Failed", Content = tostring(result), Duration = 5 }) end)
        end
    end)
end })
