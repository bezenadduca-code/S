...
local function performCurvedDash(targetRoot)
    ...
    -- If behind the target, instantly snap to front so the arc always looks clean
    local behindDot = targetRoot.CFrame.LookVector:Dot((myPos - targetRoot.Position).Unit)
    if behindDot < -0.5 then
