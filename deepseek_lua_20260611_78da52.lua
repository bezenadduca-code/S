...
• NEW: Instant hitbox-based auto block — zero delay, syncs with detection circles
]],
...
-- Settings table
combatS = combatS or {}
combatS.autoBlockOn = false
combatS.blockType = "Block"
combatS.detectionRange = 16
combatS.blockCooldown = 0.12
...
-- Target hitbox size (standard Forsaken hitbox)
local TARGET_HITBOX_SIZE = Vector3.new(4.50, 6.00, 7.50)
local SIZE_MARGIN = 0.05

local function matchesTargetSize(size)
    return math.abs(size.X - TARGET_HITBOX_SIZE.X) <= SIZE_MARGIN
       and math.abs(size.Y - TARGET_HITBOX_SIZE.Y) <= SIZE_MARGIN
       and math.abs(size.Z - TARGET_HITBOX_SIZE.Z) <= SIZE_MARGIN
end

local function cacheHRP(char)
    instantBlock.cachedHRP = char and char:FindFirstChild("HumanoidRootPart")
end

local function fireBlockWithDelay()
    local function doFire()
        if combatS.blockType == "Block" then
            combatFireAbility("Block")
            if combatS.doubleBlock then 
                task.wait(0.05)
...
local function attemptBlock(hitbox)
    if not instantBlock.enabled then return end
    if not combatS.autoBlockOn then return end
    if not hitbox or not hitbox:IsA("BasePart") then return end
    if not matchesTargetSize(hitbox.Size) then return end
    if not instantBlock.cachedHRP then return end
    
    local distance = (hitbox.Position - instantBlock.cachedHRP.Position).Magnitude
    if distance > combatS.detectionRange then return end
    
    if combatS.facingCheck then
        local killer = hitbox:FindFirstAncestorOfClass("Model")
        if killer then
            local killerHRP = killer:FindFirstChild("HumanoidRootPart")
            if killerHRP and not combatIsFacing(instantBlock.cachedHRP, killerHRP, killer.Name) then
                return
            end
        end
    end
    
    if combatS.abMissChance > 0 then
        if math.random(1, 100) <= combatS.abMissChance then
            return
        end
    end
    
    local now = tick()
    if now >= instantBlock.cooldownEnd then
        instantBlock.cooldownEnd = now + instantBlock.cooldownTime
        instantBlock.lastBlockTime = now
        fireBlockWithDelay()
    end
end

local function setupHitboxListener()
    local success, folder = pcall(function()
        return workspace:WaitForChild("Hitboxes", 10)
    end)
    
    if not success or not folder then
        task.delay(2, setupHitboxListener)
        return
    end
    
    instantBlock.hitboxFolder = folder
    
    for _, hb in ipairs(folder:GetChildren()) do
        task.spawn(attemptBlock, hb)
    end
    
    local childAddedConn = folder.ChildAdded:Connect(function(hb)
        task.spawn(attemptBlock, hb)
    end)
    table.insert(instantBlock.connections, childAddedConn)
    
    local heartbeatConn = svc.Run.Heartbeat:Connect(function()
        if not instantBlock.enabled or not combatS.autoBlockOn then return end
        if not instantBlock.cachedHRP then return end
        
        for _, hb in ipairs(folder:GetChildren()) do
            if hb:IsA("BasePart") and matchesTargetSize(hb.Size) then
                local distance = (hb.Position - instantBlock.cachedHRP.Position).Magnitude
                if distance <= combatS.detectionRange then
                    local now = tick()
                    if now >= instantBlock.cooldownEnd then
                        instantBlock.cooldownEnd = now + instantBlock.cooldownTime
                        instantBlock.lastBlockTime = now
    ...
    table.insert(instantBlock.connections, heartbeatConn)
end

local function startInstantBlock()
    if instantBlock.enabled then return end
    instantBlock.enabled = true
    
    if lp.Character then
        cacheHRP(lp.Character)
    end
    
    setupHitboxListener()
end

local function stopInstantBlock()
    instantBlock.enabled = false
    for _, conn in ipairs(instantBlock.connections) do
        pcall(function() conn:Disconnect() end)
    end
    instantBlock.connections = {}
    instantBlock.hitboxFolder = nil
end

local function syncCooldown(value)
    instantBlock.cooldownTime = math.max(0.05, value)
end

lp.CharacterAdded:Connect(function(char)
    task.wait(0.5)
    cacheHRP(char)
    if instantBlock.enabled and combatS.autoBlockOn then
        stopInstantBlock()
        task.wait(0.2)
        setupHitboxListener()
        instantBlock.enabled = true
        ...
        if hrp then
            if combatS.killerCircles then
                ...
                if combatCircles[k] then
                    pcall(function() combatCircles[k]:Destroy() end)
                    combatCircles[k]=nil
                end
            end
        end
        ...
        if hrp then
            if combatS.facingVisual then
                ...
                if combatFacingVisuals[k] then
                    pcall(function() combatFacingVisuals[k]:Destroy() end)
...
local function combatStartLoops()
    ...
    combatVisualTickConn = svc.Run.Heartbeat:Connect(function()
        _visualThrottle = _visualThrottle + 1
...
-- UI Section
local sec_guest = tabGuest1337:Section({ Title = "Auto Block (Instant Hitbox)", Opened = true })
...
sec_guest:Toggle({ 
    ...
    Callback = function(on) 
        ...
        if on then 
            startInstantBlock()
            if combatS.killerCircles or combatS.facingVisual then
                combatStartLoops()
            end
        else 
            stopInstantBlock()
...
sec_guest:Slider({ 
    ...
    Callback = function(v) 
        combatS.detectionRange = v
        if combatS.killerCircles then combatUpdateCircles() end
    end 
})

sec_guest:Slider({ 
    Title = "Block Cooldown (s)", 
    Flag = "guestBlockCooldown", 
    Step = 0.01,
    Value = {Min = 0.05, Max = 0.5, Default = 0.12}, 
    Callback = function(v) 
        syncCooldown(v)
    end 
})
...
sec_guest:Toggle({ 
    ...
    Callback = function(on)
        ...
        if on then combatStartLoops() else combatUpdateFacing() end
    end, 
    Type = "Checkbox"
})
...
sec_guest:Paragraph({
    Title = "How it works",
    Content = "Detects killer hitboxes the moment they spawn. Zero-delay reaction time. Range syncs with Detection Circles. Cooldown prevents spam. Block Delay adds latency before firing."
})
...
print("TEST 2")
print("Hutao [forsaken] ready")

print("[hutao] writefile:", writefile ~= nil)
print("[hutao] readfile: ", readfile  ~= nil)
print("[hutao] isfile:   ", isfile    ~= nil)

task.spawn(function()
    task.wait(1.5)

    print("[hutao] Attempting Load()...")
    local loadOk, loadErr = pcall(function() sakiConfig:Load() end)
    if loadOk then
        print("[hutao] Load() succeeded")
    else
        print("[hutao] Load() FAILED:", tostring(loadErr))
    end

    task.defer(function()
        print("[hutao] Syncing flags and starting saved features...")
        ...
        -- Guest1337 auto block
        combatS.autoBlockOn = win.Flags.guestAutoBlock or false
        if combatS.autoBlockOn then startInstantBlock() end
        
        combatS.killerCircles = win.Flags.guestCircles or false
        combatS.facingVisual = win.Flags.guestFacingVis or false
        if combatS.killerCircles or combatS.facingVisual then combatStartLoops() end

        print("[hutao] Feature restore complete.")
    end)
end)
...
task.spawn(function()
    while true do
        ...
        if not saveOk then
            print("[hutao] Save() FAILED:", tostring(saveErr))
        end
    end
end)
