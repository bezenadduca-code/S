...
local function combatHDTBeginDrag(killerModel, blockTrack)
    pcall(function()
        ...
        -- DO NOT disable controls — we need the mobile control system
        -- alive so our fake touch input actually drives movement.
        -- We still freeze the sprint module so speed stays at hdtMoveSpeed.
        pcall(function()
            if sprintMod then
                sprintMod.SprintSpeed    = combatS.hdtMoveSpeed
                sprintMod.MaxSprintSpeed = combatS.hdtMoveSpeed
            end
        end)
        if hum and hum.Parent then
        ...
        -- ── MOBILE THUMBSTICK FAKE-TOUCH PUSH ────────────────────────────
        -- Instead of hum:MoveTo() we simulate a finger pressing the
        -- DynamicThumbstick all the way forward.  This goes through the
        -- real Roblox mobile control pipeline, so movement is smooth and
        -- physics-correct — exactly like the player doing it themselves.
        --
        -- Strategy:
        --   1. Find the DynamicThumbstick frame in TouchGui.
        --   2. Fire a TouchBegan at the centre of the stick (activates it).
        --   3. Every frame fire a TouchMoved to the top of the stick
        --      (full forward = toward the killer).
        --   4. On stop, fire TouchEnded to release cleanly.
        --
        -- All three events are fired via the Frame's InputBegan /
        -- InputChanged / InputEnded signals that the ControlModule listens
        -- to internally — we replicate what a real finger would produce.
        -- ─────────────────────────────────────────────────────────────────

        local dtFrame        = nil   -- the DynamicThumbstick Frame
        local fakeInputObj   = nil   -- synthetic InputObject we reuse
        local touchConn      = nil   -- RenderStepped connection for the push loop

        local function hdtFindThumbstick()
            local tg  = gui:FindFirstChild("TouchGui");          if not tg  then return nil end
            local tcf = tg:FindFirstChild("TouchControlFrame");  if not tcf then return nil end
            return tcf:FindFirstChild("DynamicThumbstick")
        end

        -- Build a fake InputObject table that mimics what UIS passes
        -- to GuiObject.InputBegan / InputChanged / InputEnded.
        local function hdtMakeFakeInput(pos)
            -- We use a real InputObject from UserInputService if we can
            -- grab one; otherwise we build a plain table that duck-types.
            local obj = {
        ...
        -- Work out which screen position equals "full forward" on the stick.
        -- "Forward" on mobile = toward the killer in world-space, projected
        -- onto the thumbstick's 2-D axes.  We do this each frame so it
        -- tracks the killer even if they strafe.
        local function hdtGetForwardScreenPos(dt)
            -- Absolute centre of the DynamicThumbstick on screen
            local absPos  = dt.AbsolutePosition                  -- top-left corner
            local absSize = dt.AbsoluteSize
            local centre  = absPos + absSize * 0.5

            -- Direction from our HRP toward killer in world space (XZ plane)
            local myPos  = hrp and hrp.Position or Vector3.zero
            local kPos   = killerRoot and killerRoot.Position or myPos
            local dir    = Vector3.new(kPos.X - myPos.X, 0, kPos.Z - myPos.Z)

            local radius = absSize.X * 0.5   -- push to the edge = full speed

            if dir.Magnitude < 0.01 then
                -- Killer is directly on top — just push straight up
                return Vector2.new(centre.X, centre.Y - radius)
            end

            dir = dir.Unit

            -- Map world XZ → screen XY using the camera's CFrame so the
            -- stick feels natural regardless of camera angle.
            local cam    = svc.WS.CurrentCamera
            local camCF  = cam and cam.CFrame or CFrame.new()
            local camRight   = camCF.RightVector
            local camForward = -camCF.LookVector   -- -Z = forward in camera space

            -- Flatten to XZ
            local fwdFlat  = Vector3.new(camForward.X, 0, camForward.Z)
            local rightFlat= Vector3.new(camRight.X,   0, camRight.Z)

            local fwdMag   = fwdFlat.Magnitude
            local rightMag = rightFlat.Magnitude
            if fwdMag   < 0.001 then fwdFlat   = Vector3.new(0,0,-1) else fwdFlat   = fwdFlat   / fwdMag   end
            if rightMag < 0.001 then rightFlat  = Vector3.new(1,0, 0) else rightFlat = rightFlat / rightMag end

            local screenX =  dir:Dot(rightFlat)   -- positive = right
            local screenY = -dir:Dot(fwdFlat)     -- negative = up on screen (Y grows downward)

            -- Clamp to unit circle, then scale to stick radius
            local len = math.sqrt(screenX*screenX + screenY*screenY)
            if len > 1 then screenX = screenX/len; screenY = screenY/len end

            return Vector2.new(
                centre.X + screenX * radius,
                centre.Y + screenY * radius
            )
        end

        pcall(function()
            dtFrame = hdtFindThumbstick()
            if not dtFrame then return end

            local absPos  = dtFrame.AbsolutePosition
            local absSize = dtFrame.AbsoluteSize
            local centreVec3 = Vector3.new(
                absPos.X + absSize.X * 0.5,
                absPos.Y + absSize.Y * 0.5,
                0
            )

            -- Fire TouchBegan at the centre to activate the thumbstick
            fakeInputObj = hdtMakeFakeInput(centreVec3)
            dtFrame:FindFirstChildOfClass("ImageButton") -- just a probe; event fires below
            pcall(function() dtFrame.InputBegan:Fire(fakeInputObj) end)

            -- Every frame: update position toward killer and fire TouchMoved
            touchConn = svc.Run.RenderStepped:Connect(function()
                if stopped or not dtFrame or not dtFrame.Parent then return end
                local fwdPos2 = hdtGetForwardScreenPos(dtFrame)
                fakeInputObj.UserInputState = Enum.UserInputState.Change
                fakeInputObj.Position = Vector3.new(fwdPos2.X, fwdPos2.Y, 0)
                pcall(function() dtFrame.InputChanged:Fire(fakeInputObj) end)
            end)
        end)
        -- ─────────────────────────────────────────────────────────────────
        ...
        local function stopMove()
            ...
            stopped = true
            -- Disconnect the per-frame push first
            if touchConn then pcall(function() touchConn:Disconnect() end); touchConn = nil end
            -- Release the fake touch so the stick snaps back to centre
            pcall(function()
                if dtFrame and fakeInputObj then
                    fakeInputObj.UserInputState = Enum.UserInputState.End
                    dtFrame.InputEnded:Fire(fakeInputObj)
                end
            end)
            -- Re-enable controls (we left them on, but re-enable just in case)
            pcall(function() if controls then controls:Enable() end end)
            restoreMovement()
        end
