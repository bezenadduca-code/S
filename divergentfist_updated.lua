...
-- ──────────────────────────────────────────────
--  MOVE CONTROLLER (360 ORBIT → LAND AT BACK)
-- ──────────────────────────────────────────────
local function performCurvedDash(targetRoot)
    ...
    -- Randomize spin direction: 1 = clockwise, -1 = counter-clockwise
    local spinSign = (math.random(1, 2) == 1) and 1 or -1
    local dashDirection = (spinSign == 1) and "Left" or "Right"
    ...
    -- 75% of time = full 360 orbit, 25% = glide into back position
    local SPIN_PHASE = 0.75
    local APPROACH_PHASE = 1 - SPIN_PHASE
    ...
    -- Capture entry angle once at dash start so the orbit is stable
    local entryVector = (myPos - targetRoot.Position)
    local entryAngle = math.atan2(entryVector.X, entryVector.Z)

    -- The angle that corresponds to directly behind the target
    local backVec = targetRoot.CFrame.LookVector * -1
    local backAngle = math.atan2(backVec.X, backVec.Z)

    -- How many extra radians (beyond the full 360) to reach the back angle,
    -- going in our chosen spin direction
    local delta = (backAngle - entryAngle) * spinSign
    if delta < 0 then delta = delta + math.pi * 2 end
    -- Total sweep = one full loop + the extra to land exactly at back
    local totalAngle = (math.pi * 2) + delta

    connection = RunService.Heartbeat:Connect(function(dt)
        if not targetRoot or not targetRoot.Parent or elapsed >= totalDuration then
            connection:Disconnect()
            -- Hard snap to exact back position facing the target
        ...
        local frameTargetPos

        if t <= SPIN_PHASE then
            -- Phase 1: orbit at constant radius for the full 360+delta sweep
            local spinT = t / SPIN_PHASE  -- 0→1 over spin phase
            local angle = entryAngle + (spinSign * spinT * totalAngle)
            local orbitPos = currentTargetPos + Vector3.new(
                math.sin(angle) * CONFIG.LoopRadius,
                0,
                math.cos(angle) * CONFIG.LoopRadius
            )
            frameTargetPos = Vector3.new(orbitPos.X, hrp.Position.Y, orbitPos.Z)
        else
            -- Phase 2: smoothly glide from orbit end-point into the back position
            local approachT = (t - SPIN_PHASE) / APPROACH_PHASE  -- 0→1
            local smoothT = approachT * approachT * (3 - 2 * approachT)  -- smoothstep

            -- Where the orbit finished (end of the full sweep)
            local endAngle = entryAngle + (spinSign * totalAngle)
            local spinEndPos = currentTargetPos + Vector3.new(
                math.sin(endAngle) * CONFIG.LoopRadius,
                0,
                math.cos(endAngle) * CONFIG.LoopRadius
            )
            local backPos = (targetRoot.CFrame * CFrame.new(0, 0, CONFIG.BehindOffset)).Position
            local lerped = spinEndPos:Lerp(backPos, smoothT)
            frameTargetPos = Vector3.new(lerped.X, hrp.Position.Y, lerped.Z)
        end
