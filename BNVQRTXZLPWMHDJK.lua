...
ui:Popup({
    Title   = GradientText("hutao [forsaken]", Colors.Pink, Colors.Gold),
    Icon    = "sparkles",
    Content = GradientText("maintained by mitsuki", Colors.Gold, Colors.LightBlue),
    Buttons = {
        { Title = "Exit",     Callback = function() end,                Variant = "Tertiary" },
        { Title = GradientText("Continue", Colors.Toxic, Colors.Orange), Callback = function() popupDone = true end, Variant = "Primary" },
    },
})
...
local _tagFrameCount, _tagLastTick = 0, tick()
svc.Run.RenderStepped:Connect(function()
    _tagFrameCount = _tagFrameCount + 1
    local now = tick()
    if now - _tagLastTick >= 1 then
        local fps = math.floor(_tagFrameCount / (now - _tagLastTick))
